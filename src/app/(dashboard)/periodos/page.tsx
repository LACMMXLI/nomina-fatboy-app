...
export default async function PeriodsPage() {
  ...
  const rows = await loadReportRows(user, {
    extra: { periodId: { in: periods.map((period) => period.id) } },
  });
  const byPeriod = summarizeBy(rows, (row) => row.periodId);
  return (
    <>
      ...
      <div className="table-wrap"><table className="data-table"><thead><tr><th>Folio</th><th>Periodo</th><th>Fechas</th><th>Sucursal</th><th>Estado</th><th>Empleados</th>{canSeeSalary && <th>Pagado</th>}{canSeeSalary && <th>Pendiente</th>}</tr></thead><tbody>
        {periods.map((period) => {
          const totals = byPeriod.get(period.id) ?? EMPTY_TOTALS;
          return <tr key={period.id}><td><Link className="font-semibold text-red-600" href={`/periodos/${period.id}`}>{period.folio}</Link></td><td>{period.name}</td><td>{formatDate(period.startDate)} – {formatDate(period.endDate)}</td><td>{period.branch?.name ?? "Todas"}</td><td><Badge value={period.status} /></td><td>{totals.empleados}</td>{canSeeSalary && <td>{formatMoney(totals.pagado)}</td>}{canSeeSalary && <td>{formatMoney(totals.pendiente)}</td>}</tr>;
        })}
