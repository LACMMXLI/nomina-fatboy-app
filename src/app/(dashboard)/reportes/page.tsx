...
import { EMPTY_TOTALS, loadReportRows, reportWhere, summarize, summarizeBy, summarizeConcepts } from "@/server/reporting";
...
export default async function ReportsPage() {
  ...
  const [rows, branches, conceptItems] = await Promise.all([
    loadReportRows(user),
    db.branch.findMany({ where: user.role === "SUPER_ADMIN" ? {} : { id: { in: user.branchIds } }, select: { id: true, name: true }, orderBy: { name: "asc" } }),
    db.payrollItem.findMany({ where: { payroll: where }, select: { conceptNameSnapshot: true, type: true, totalAmount: true } }),
  ]);
  const totals = summarize(rows);
  const byBranch = summarizeBy(rows, (row) => row.branchId);
  const byConcept = summarizeConcepts(conceptItems);
  return (
    <>
